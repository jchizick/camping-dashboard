-- Owner-only, token-free management read. No table, policy or private-function changes.
-- Reuse the existing DB-authorized identity resolver; never return its user_id.
...
begin
  ...
  if p_operation = 'list_access' then
    if jsonb_typeof(p_input) is distinct from 'object'
      or p_input - 'tripId' <> '{}'::jsonb
      or jsonb_typeof(p_input->'tripId') is distinct from 'string'
      or p_input->>'tripId' !~ '^[A-Za-z0-9_-]{1,200}$' then
      raise exception using errcode='22023',message='Invalid access request';
    end if;
    -- Same parent lock as membership mutations. Ownership is rechecked on every call.
    perform app_private.lock_owned_trip(p_input->>'tripId');
    select jsonb_build_object(
      'people',coalesce((select jsonb_agg(jsonb_build_object(
        'membershipId',m.id,'email',nullif(btrim(m.email),''),'role',m.role,
        'isCurrentUser',m.is_current_user)
        order by case m.role when 'owner' then 0 when 'editor' then 1 else 2 end,
          lower(coalesce(m.email,'')) collate "C",m.id)
        from app_private.list_trip_access(p_input->>'tripId') m),'[]'::jsonb),
      'pendingInvitations',coalesce((select jsonb_agg(jsonb_build_object(
        'invitationId',i.id,'email',i.invited_email_normalized,'role',i.role,
        'status',i.status,'createdAt',i.created_at,'expiresAt',i.expires_at)
        order by i.created_at desc,i.id)
        from public.trip_invitations i
        where i.trip_id=p_input->>'tripId' and i.status='pending'
          and i.expires_at>clock_timestamp()
          and exists(select 1 from public.trip_members inviter
            where inviter.trip_id=i.trip_id and inviter.user_id=i.invited_by and inviter.role='owner')),'[]'::jsonb)
    ) into v_result;
    -- Do not call list_trip_invitations: that legacy primitive writes expiry statuses.
    perform set_config('request.jwt.claims',coalesce(v_claims,''),true);
    perform set_config('request.jwt.claim.sub',coalesce(v_sub,''),true);
    return v_result;
  end if;
