...
import {SCHEMA_SQL,POST_SENTINELS} from './postMigrationContract.mjs';
import {projectMatches,provenance,PROTECTED_SCHEMAS} from './sqlIdentity.mjs';
import {describeCatalog,compareCatalogs} from './catalogComparison.mjs';
import {ACCESS_REMOVAL_VERSIONS as VERSIONS,ACCESS_REMOVAL_FINGERPRINT as SCHEMA_FINGERPRINT} from './accessRemovalContract.mjs';
import {CA_HASH,TARGETS} from './repairContract.mjs';
import {schema as schema33} from './accessBridgeContract.mjs';
import {VERSION,FILE,FILE_HASH} from './accessManagementInventory.mjs';
export {CA_HASH,VERSION,FILE,FILE_HASH};
export const FINAL_VERSIONS=[...VERSIONS,VERSION];
export const FINAL_FINGERPRINT='3c6049625e9eb174cbe148eb44592da0';
export const MODE='STAGING_ACCESS_MANAGEMENT_33_TO_34',REF='mgnkvfohpqixgacszovv';
export const PRE='POST_MIGRATION_33_PRE_ACCESS_MANAGEMENT',FINAL='POST_ACCESS_MANAGEMENT_STAGING_34';
...
export const PRE_DEFINITION='cce6bb5918b36a87d4c9ec4a8567ebb3370f35c95a144366be69df23d2e5201c';
export const FINAL_DEFINITION='71379fccb8ab668a8673fa5b8ad923d363c53106990aee518e50f79122956b6e';
export const PRIVATE_DEFINITION='3c1057ec851fa1367eeee25053038877c5085d3511706aa62fe0515f078b1de1';
const MANIFESTS=['da7e8fae024607933de690d833940948d0b141d5ccacd7e7837fd14c6c7fda74','08d5f1f40e1ef8e1a28b38c44211876938d58a03e6688b974d42950ce16e9d51'];
export const hash=s=>createHash('sha256').update(s).digest('hex');
export const fail=code=>{throw Error('ACCESS_MANAGEMENT_'+code);};
export const same=(a,b)=>JSON.stringify(a)===JSON.stringify(b);
export function migrationBytes(bytes){if(hash(bytes)!==FILE_HASH)fail('MIGRATION_FILE_MISMATCH');}
export function startHistory(h){if(same(h,FINAL_VERSIONS))fail('ALREADY_COMPLETE');if(!same(h,VERSIONS))fail('START_HISTORY_MISMATCH');}
...
export function schema(snapshot,final=false){
 ...
 if(!same(r?.migration_history,final?FINAL_VERSIONS:VERSIONS))fail(final?'POST_HISTORY_FAILED':'START_HISTORY_MISMATCH');
 ...
 const fingerprint=final?FINAL_FINGERPRINT:SCHEMA_FINGERPRINT;
 ...
 if(!final){try{schema33(snapshot,true);}catch{fail('PRE_SCHEMA_FAILED');}} // Never relax the frozen final-33 verifier.
 return desc;
}
...
async function requireTypes(io){
 try{const result=await io.types();if(result?.normalizedEquivalent!==true||result?.structuralEquivalent!==true)fail('TYPE_GATE_FAILED');return result;}catch{fail('TYPE_GATE_FAILED');}
}
export async function runAccessManagement({mode,ref,session,source,io,now=Date.now,action='--dry-run'}){
 ...
 if(action==='--verify'){schema(first.a,true);const types=await requireTypes(io);return {result:'FINAL_34_PASS',state:FINAL,types,mutation:false,deploymentAuthorized:false};}
 if(same(first.a.row.migration_history,FINAL_VERSIONS)){schema(first.a,true);return {result:'ACCESS_MANAGEMENT_ALREADY_COMPLETE',mutation:false,deploymentAuthorized:false};}
 startHistory(first.a.row.migration_history);schema(first.a);await requireTypes(io);
 ...
 migrationBytes(await io.bytes());let latestPending;try{latestPending=await io.pending(latest.a);}catch{fail('DRY_RUN_FAILED');}pendingSet(latestPending);
 ...
 const types=await requireTypes(io);
 const result={result:'PASS',state:FINAL,history:after.a.row.migration_history,fingerprint:FINAL_FINGERPRINT,mutation:true,types,deploymentAuthorized:false};
...
export function transactionSql(bytes){
 ...
 IF (SELECT coalesce(json_agg(version ORDER BY version)::jsonb,'[]') FROM supabase_migrations.schema_migrations) <> '${JSON.stringify(VERSIONS)}'::jsonb THEN RAISE EXCEPTION 'ACCESS_MANAGEMENT_START_HISTORY_MISMATCH'; END IF;
 IF (SELECT fingerprint FROM (${SCHEMA_SQL}) c) <> '${SCHEMA_FINGERPRINT}' THEN RAISE EXCEPTION 'ACCESS_MANAGEMENT_PRE_CATALOG_FAILED'; END IF;
 IF (SELECT proowner FROM pg_proc WHERE oid='public.trip_invitation_bridge(uuid,text,jsonb)'::regprocedure) <> 'postgres'::regrole THEN RAISE EXCEPTION 'ACCESS_MANAGEMENT_UNEXPECTED_GRANT'; END IF;
 ...
 THEN RAISE EXCEPTION 'ACCESS_MANAGEMENT_UNEXPECTED_GRANT'; END IF;
 END LOOP;
 END $access_gate$;`;
 ...
 SELECT pg_advisory_xact_lock(330034); LOCK TABLE supabase_migrations.schema_migrations IN EXCLUSIVE MODE;
 ${check}
 ${body}
 INSERT INTO supabase_migrations.schema_migrations(version,statements,name) VALUES ('${VERSION}',ARRAY[$access_migration$${body}$access_migration$],'trip_access_management_read');
 DO $access_final$ BEGIN IF (SELECT fingerprint FROM (${SCHEMA_SQL}) c) <> '${FINAL_FINGERPRINT}' THEN RAISE EXCEPTION 'ACCESS_MANAGEMENT_FINAL_CATALOG_FAILED'; END IF; END $access_final$;
 COMMIT;`;
}
