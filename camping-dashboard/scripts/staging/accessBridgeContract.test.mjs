...
import {accessRemovalInventory,ACCESS_REMOVAL_VERSIONS} from './accessRemovalContract.mjs';
import {sourcePath} from './paths.mjs';
import {VERSIONS,authorizeMigrationAction} from './postMigrationContract.mjs';
import {MODE,REF,BRIDGE,PRIVATE,runAccessBridge,startHistory,schema,transactionSql} from './accessBridgeContract.mjs';
import {runHostedAccessBridge} from './accessBridgeTransport.mjs';
import {sourceFiles,requireCommittedSource} from './accessBridgeSource.mjs';
if(!process.env.ACCESS_BRIDGE_LOCAL_FIXTURE)throw Error('RUN_VALIDATE_ACCESS_BRIDGE_FOR_FRESH_LOCAL_FIXTURE');
const {before,after,source}=JSON.parse(readFileSync(process.env.ACCESS_BRIDGE_LOCAL_FIXTURE,'utf8'));
const bytes=accessRemovalInventory(sourcePath()).at(-1).bytes,session='synthetic-current-session',now=Date.now();
const bundle=s=>({...repairBundle(s,session,now),source});
function harness(change=()=>{}){let applied=0,captured=0;const io={bytes:async()=>bytes,preserve:async()=>{},pending:async()=>[ACCESS_REMOVAL_VERSIONS.at(-1)],apply:async()=>{applied++;},capture:async()=>{const b=bundle(applied?after:before);change(b,++captured);return b;}};return {io,get applied(){return applied;}};}
const run=(h,options={})=>runAccessBridge({mode:MODE,ref:REF,session,source,io:h.io,action:'--apply',now:()=>now,...options});
const changeSnapshot=(b,fn)=>{fn(b.a);b.b=structuredClone(b.a);};
test('fresh replay produced exact start and final contracts',()=>{schema(before);schema(after,true);assert.deepEqual(before.row.migration_history,VERSIONS);assert.deepEqual(after.row.migration_history,ACCESS_REMOVAL_VERSIONS);});
...
test('apply performs exactly one write after fresh in-process dry-run',async()=>{const h=harness();assert.equal((await run(h)).state,'POST_ACCESS_REMOVAL_STAGING_33');assert.equal(h.applied,1);});
test('already complete is structured and never reapplies',async()=>{const h=harness(b=>{b.a=structuredClone(after);b.b=structuredClone(after);});assert.equal((await run(h)).result,'ACCESS_BRIDGE_ALREADY_COMPLETE');assert.equal(h.applied,0);});
...
for(const ref of ['gdsmyxzqtmhwbcyobzou','rmfhueseulevyvetblnn','abcdefghijklmnopqrst'])test('protected or wrong target '+ref,async()=>{const h=harness();await assert.rejects(run(h,{ref}),/TARGET_MISMATCH/);assert.equal(h.applied,0);await assert.rejects(runHostedAccessBridge(MODE,'--apply',{STAGING_REF:ref}),/TARGET_MISMATCH/);});
const cases=[
 ['production28',b=>changeSnapshot(b,s=>s.row.migration_history=VERSIONS.slice(0,28)),'START_HISTORY_MISMATCH'],
 ['31',b=>changeSnapshot(b,s=>s.row.migration_history=VERSIONS.slice(0,31)),'START_HISTORY_MISMATCH'],
 ['wrong32',b=>changeSnapshot(b,s=>s.row.migration_history[31]='20990101000000'),'START_HISTORY_MISMATCH'],
...
test('unknown apply outcome is sanitized and never retried',async()=>{const h=harness();let n=0;h.io.apply=async()=>{n++;throw Error('SECRET');};await assert.rejects(run(h),/^Error: ACCESS_BRIDGE_APPLY_FAILED$/);assert.equal(n,1);});
test('dry-run failure sanitized',async()=>{const h=harness();h.io.pending=async()=>{throw Error('SECRET');};await assert.rejects(run(h),/^Error: ACCESS_BRIDGE_DRY_RUN_FAILED$/);assert.equal(h.applied,0);});
test('transaction has one fixed migration, atomic history, lock and rollback guards',()=>{const s=transactionSql(bytes);assert.equal((s.match(/INSERT INTO supabase_migrations.schema_migrations/g)||[]).length,1);assert.ok(s.indexOf('pg_advisory_xact_lock')<s.indexOf(bytes.toString()));assert.ok(s.includes('ACCESS_BRIDGE_START_HISTORY_MISMATCH'));assert.ok(s.endsWith('COMMIT;'));assert.throws(()=>transactionSql(Buffer.from('arbitrary')),/MIGRATION_FILE_MISMATCH/);});
test('generic mutation remains disabled',()=>{for(const a of ['plan','apply'])assert.throws(()=>authorizeMigrationAction(a),/VERIFY_ONLY/);});
test('source closure includes migrations, types and SQL without ignored artifacts',()=>{const paths=sourceFiles();assert.ok(paths.includes('scripts/staging/schemaFingerprint.sql'));assert.ok(paths.includes('src/types/supabase.ts'));assert.equal(paths.filter(p=>p.startsWith('supabase/migrations/')).length,33);assert.ok(paths.every(p=>!p.startsWith('output/')));});
...
test('modified committed dependency cannot authorize',()=>assert.throws(()=>requireCommittedSource(source,sourcePath(),gitFixture((name,b)=>name.endsWith('accessBridgeTransport.mjs')?Buffer.from('changed'):b)),/UNCOMMITTED_DEPENDENCY/));
test('migration blob must itself be the exact reviewed bytes',()=>assert.throws(()=>requireCommittedSource(source,sourcePath(),gitFixture((name,b)=>name.endsWith('trip_access_removal_bridge.sql')?Buffer.concat([b,Buffer.from('\n')]):b)),/MIGRATION_FILE_MISMATCH/));
test('bare CLI and aliases reject without network',()=>{const cli=fileURLToPath(new URL('./access-bridge-32-to-33.mjs',import.meta.url));for(const args of [[],['plan'],['apply'],[MODE]])assert.throws(()=>execFileSync(process.execPath,[cli,...args],{stdio:'pipe'}),e=>String(e.stdout).includes('MODE_NOT_EXPLICIT'));});
