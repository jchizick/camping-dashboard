...
import {accessRemovalInventory,verifyAccessRemovalTypes} from './accessRemovalContract.mjs';
import {SNAPSHOT_SQL} from './accessBridgeTransport.mjs';
import {parseSnapshot} from './repairTransport.mjs';
import {repairBundle} from './fixtures/repair.mjs';
import {MODE,REF,FILE,VERSION,runAccessBridge,transactionSql,schema} from './accessBridgeContract.mjs';
if(process.argv.length!==3||process.argv[2]!=='--reset-disposable')throw Error('EXPLICIT_LOCAL_RESET_REQUIRED');
const inventory=accessRemovalInventory(sourcePath()),bytes=inventory.at(-1).bytes;
...
const dir=mkdtempSync(join(tmpdir(),'fp-access-bridge-local-'));
const session=randomUUID(),source='a'.repeat(40);let applies=0;
try{
 mkdirSync(join(dir,'supabase/migrations'),{recursive:true});
 for(const e of inventory.slice(0,32))writeFileSync(join(dir,'supabase/migrations',e.name),e.bytes);
 ...
 const io={bytes:async()=>bytes,preserve:async()=>{},capture:async()=>({...repairBundle(capture(),session),source}),
 ...
 const dry=await runAccessBridge({...args,action:'--dry-run'});assert.deepEqual(dry.pending,[VERSION]);assert.equal(applies,0);
 const result=await runAccessBridge({...args,action:'--apply'});assert.equal(applies,1);
 const after=capture();schema(after,true);
 const repeated=await runAccessBridge({...args,action:'--apply'});assert.equal(repeated.result,'ACCESS_BRIDGE_ALREADY_COMPLETE');assert.equal(applies,1);
 ...
 const tests=execFileSync(process.execPath,['--test',sourcePath('scripts/staging/accessBridgeContract.test.mjs')],{cwd:sourcePath(),env:{...process.env,ACCESS_BRIDGE_LOCAL_FIXTURE:fixture},encoding:'utf8',stdio:'pipe',timeout:120000});
 console.log(tests);
 const out=sourcePath('output/access-bridge-validation');mkdirSync(out,{recursive:true});
 writeFileSync(join(out,'generated33.ts'),generated);writeFileSync(join(out,'result.json'),JSON.stringify({result,types,dry,applyCount:applies,repeated,controlPlane:'SYNTHETIC_LOCAL_ONLY'},null,2));
 console.log(JSON.stringify({result:'LOCAL_32_TO_33_PASS',applyCount:applies,history:after.row.migration_history.length,types,repeated:repeated.result}));
}finally{const rel=relative(resolve(tmpdir()),resolve(dir));if(!rel.startsWith('..')&&!isAbsolute(rel)&&rel.startsWith('fp-access-bridge-local-'))rmSync(dir,{recursive:true,force:true});}
