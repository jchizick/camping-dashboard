...
import {requireCommittedSource} from './accessBridgeSource.mjs';
import {MODE,REF,FILE,CA_HASH,hash,fail,runAccessBridge,transactionSql} from './accessBridgeContract.mjs';
...
function transport(env){
 ...
 const bytes=()=>{requireCommittedSource(source);const entries=accessRemovalInventory(SOURCE_ROOT);return entries.find(e=>e.name===FILE).bytes;};
 ...
 return {source,bytes,capture,
  pending:async snapshot=>{const entries=accessRemovalInventory(SOURCE_ROOT);return entries.map(e=>e.name.slice(0,14)).filter(v=>!snapshot.row.migration_history.includes(v));},
  preserve:async result=>{mkdirSync(env.STAGING_EVIDENCE_DIR,{recursive:true});writeFileSync(join(env.STAGING_EVIDENCE_DIR,'access-bridge-result.json'),JSON.stringify(result,null,2));},
  apply:async()=>{
   ...
   options.env.PGOPTIONS='-c statement_timeout=30000';options.env.PGAPPNAME='staging-access-bridge-32-to-33';
   options.input=sql;options.timeout=60000;execFileSync(binary,args,options);
  },
 };
}
export async function runHostedAccessBridge(mode,action,env=process.env){
 if(mode!==MODE||!['--dry-run','--apply','--verify'].includes(action))fail('MODE_NOT_EXPLICIT');
 const io=transport(env);
 return runAccessBridge({mode,action,ref:REF,session:randomUUID(),source:io.source,io});
}
