...
import {FILE,FILE_HASH,hash,fail} from './accessManagementContract.mjs';
...
export function sourceFiles(root=SOURCE_ROOT){
 ...
 visit('scripts/staging/access-management-33-to-34.mjs');
 for(const path of ['package.json','package-lock.json','src/types/supabase.ts'])visit(path);
 for(const e of accessManagementInventory(root))seen.add('supabase/migrations/'+e.name);
 return [...seen].sort();
}
export function requireCommittedSource(approved,root=SOURCE_ROOT,readGit){
 ...
 }catch(e){if(/^ACCESS_MANAGEMENT_/.test(e.message))throw e;fail('SOURCE_NOT_COMMITTED');}
 return approved;
}
