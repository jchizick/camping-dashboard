...
export async function fetchDashboardData(tripId: string): Promise<DashboardData> {
    const [
        ...
        supabase.from('weather_current').select('*').eq('trip_id', tripId).single(),
        ...
        supabase.from('park_intel').select('*').eq('trip_id', tripId).single(),
        supabase.from('offline_status').select('*').eq('trip_id', tripId).single(),
        supabase.from('astro_data').select('*').eq('trip_id', tripId).single(),
